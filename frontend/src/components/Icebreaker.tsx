  ...
  if (!isConnected) {
    return (
      <div className="text-center py-6 md:py-8">
        <p className="text-sm md:text-base text-vibecaster-lavender">Please connect your wallet to use Icebreaker</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-vibecaster-lavender mb-2 flex items-center justify-center gap-2">
          <FaSnowflake className="text-blue-400" /> Icebreaker
        </h2>
        <p className="text-sm md:text-base text-vibecaster-light-purple">Break the ice with fun prompts and polls!</p>
      </div>
        ...
        <div className="bg-vibecaster-dark/50 backdrop-blur-sm rounded-lg border border-vibecaster-lavender/20">
          <div className="flex">
            <div className="px-3 md:px-4 py-2 md:py-3 text-center border-r border-vibecaster-lavender/20">
              <p className="text-lg md:text-xl font-bold text-vibecaster-lavender">{totalCategories?.toString() || '0'}</p>
              <p className="text-xs text-vibecaster-light-purple">Categories</p>
            </div>
            <div className="px-3 md:px-4 py-2 md:py-3 text-center border-r border-vibecaster-lavender/20">
              <p className="text-lg md:text-xl font-bold text-vibecaster-lavender">{totalPrompts?.toString() || '0'}</p>
            ...
            <div className="px-3 md:px-4 py-2 md:py-3 text-center border-r border-vibecaster-lavender/20 last:border-r-0">
              <p className="text-lg md:text-xl font-bold text-vibecaster-lavender">{totalPolls?.toString() || '0'}</p>
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'prompts' && (
          <div className="space-y-4">
            <h3 className="text-lg md:text-xl font-semibold text-vibecaster-lavender">Recent Prompts</h3>
            {prompts.length === 0 ? (
              <p className="text-sm md:text-base text-vibecaster-light-purple text-center py-6 md:py-8">No prompts yet. Create one to get started!</p>
            ) : (
              <div className="space-y-3 md:space-y-4">
                {prompts.slice(0, 5).map((prompt, index) => (
                  <div key={index} className="bg-vibecaster-dark/30 backdrop-blur-sm rounded-lg p-3 md:p-4">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-2 md:gap-0 mb-3">
                      <h4 className="text-sm md:text-base text-vibecaster-lavender font-medium">{prompt.text}</h4>
                      <span className="text-xs text-vibecaster-light-purple bg-vibecaster-purple-dark px-2 py-1 rounded self-start">
                        {prompt.category}
                      </span>
                    </div>
                    <p className="text-xs md:text-sm text-vibecaster-light-purple mb-3">
                      by {prompt.creator.slice(0, 6)}...{prompt.creator.slice(-4)}
                    </p>
                    <button
                      onClick={() => setSelectedPromptId(index + 1)}
                      className="bg-vibecaster-lavender text-vibecaster-dark px-3 md:px-4 py-2 rounded-md hover:bg-vibecaster-light-purple transition-colors text-sm md:text-base"
                    >
                      Respond
                    </button>
                  </div>
        ...
        {activeTab === 'polls' && (
          <div className="space-y-4">
            <h3 className="text-lg md:text-xl font-semibold text-vibecaster-lavender">Active Polls</h3>
            {polls.length === 0 ? (
              <p className="text-sm md:text-base text-vibecaster-light-purple text-center py-6 md:py-8">No polls yet. Create one to get started!</p>
            ) : (
              <div className="space-y-3 md:space-y-4">
                {polls.slice(0, 5).map((poll, index) => (
                  <div key={index} className="bg-vibecaster-dark/30 backdrop-blur-sm rounded-lg p-3 md:p-4">
                    <h4 className="text-sm md:text-base text-vibecaster-lavender font-medium mb-3">{poll.question}</h4>
                    <div className="space-y-2 mb-3">
                      {poll.options.map((option, optionIndex) => (
                        <button
                          ...
                          className="w-full text-left p-2 rounded bg-vibecaster-dark/50 hover:bg-vibecaster-lavender hover:text-vibecaster-dark transition-colors text-sm md:text-base"
                        >
                          {option}
                        </button>
                      ))}
                    </div>
                    <p className="text-xs md:text-sm text-vibecaster-light-purple">
        ...
        {activeTab === 'create' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            ...
            <div className="bg-vibecaster-dark/30 backdrop-blur-sm rounded-lg p-4 md:p-6">
              <h3 className="text-lg md:text-xl font-semibold text-vibecaster-lavender mb-3 md:mb-4">✨ Craft Icebreaker</h3>
              <div className="space-y-3 md:space-y-4">
                <div>
                  <label className="block text-sm md:text-base text-vibecaster-light-purple mb-2">Vibe Category</label>
                  <select
                    value={selectedCategory}
                    onChange={(e) => setSelectedCategory(e.target.value)}
                    className="w-full bg-vibecaster-dark/50 border border-vibecaster-lavender/20 rounded-md px-3 py-2 text-sm md:text-base text-vibecaster-lavender focus:outline-none focus:border-vibecaster-lavender"
                  >
                ...
                <div>
                  <label className="block text-sm md:text-base text-vibecaster-light-purple mb-2">Icebreaker Magic</label>
                  <textarea
                    value={promptText}
                    onChange={(e) => setPromptText(e.target.value)}
                    placeholder="Drop your fire icebreaker question here... 🔥"
                ...
                >
                  {isCreatingPrompt ? 'Crafting...' : '🚀 Launch Icebreaker'}
                </button>
              </div>
            </div>
            ...
            {/* Craft Poll */}
            <div className="bg-vibecaster-dark/30 backdrop-blur-sm rounded-lg p-4 md:p-6">
              <h3 className="text-lg md:text-xl font-semibold text-vibecaster-lavender mb-3 md:mb-4">🎯 Craft Poll</h3>
              <div className="space-y-3 md:space-y-4">
                <div>
                  <label className="block text-sm md:text-base text-vibecaster-light-purple mb-2">Poll Question</label>
                  <input
                    type="text"
                    value={pollQuestion}
                    onChange={(e) => setPollQuestion(e.target.value)}
                    placeholder="What's the tea? Drop your poll question... ☕"
                    className="w-full bg-vibecaster-dark/50 border border-vibecaster-lavender/20 rounded-md px-3 py-2 text-sm md:text-base text-vibecaster-lavender focus:outline-none focus:border-vibecaster-lavender"
                  />
                </div>
                <div>
                  <label className="block text-sm md:text-base text-vibecaster-light-purple mb-2">Vote Options</label>
                  {pollOptions.map((option, index) => (
                    <div key={index} className="flex space-x-2 mb-2">
                      <input
                        ...
                        className="flex-1 bg-vibecaster-dark/50 border border-vibecaster-lavender/20 rounded-md px-3 py-2 text-sm md:text-base text-vibecaster-lavender focus:outline-none focus:border-vibecaster-lavender"
                      />
                  ...
                  <button
                    ...
                    className="text-sm md:text-base text-vibecaster-lavender hover:text-vibecaster-light-purple transition-colors"
                  >
                    + Add Vibe Option
                  </button>
                </div>
                <button
                  ...
                  className="w-full bg-vibecaster-lavender text-vibecaster-dark py-2 rounded-md hover:bg-vibecaster-light-purple transition-colors disabled:opacity-50 text-sm md:text-base"
                >
                  {isCreatingPoll ? 'Crafting...' : '🎯 Launch Poll'}
                </button>
              </div>
            </div>
          </div>
      ...
      {selectedPromptId !== null && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-vibecaster-dark rounded-lg p-4 md:p-6 w-full max-w-md">
            <h3 className="text-lg md:text-xl font-semibold text-vibecaster-lavender mb-3 md:mb-4">Respond to Prompt</h3>
            <textarea
              value={responseText}
              onChange={(e) => setResponseText(e.target.value)}
              placeholder="Your response..."
              className="w-full bg-vibecaster-dark/50 border border-vibecaster-lavender/20 rounded-md px-3 py-2 text-sm md:text-base text-vibecaster-lavender focus:outline-none focus:border-vibecaster-lavender resize-none mb-4"
              rows={4}
            />
      ...
      {selectedPollId !== null && selectedOption !== null && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-vibecaster-dark rounded-lg p-4 md:p-6 w-full max-w-md">
            <h3 className="text-lg md:text-xl font-semibold text-vibecaster-lavender mb-3 md:mb-4">Confirm Vote</h3>
            <p className="text-sm md:text-base text-vibecaster-light-purple mb-4">
